...
public class ExampleAppIntegrationTest {
    ...
    @Rule
    public JaxRsIntegrationTestRule testContainerRule = new JaxRsIntegrationTestRule(new MyApplication());

    @Before
    public void setUpMockedAdress() {
        System.setProperty("it.ejb.url", "http://localhost:" + testContainerRule.getWmPort() +"/integration/ejb/message");
    }
    ...
    public void test_my_resource_with_valid_username() throws Exception {
        ...
        Response response = ResteasyClientBuilder.newClient()
                .target("http://localhost:"+testContainerRule.getLocalContainerPort())
